...
def split_sci(hdulist, splitrow, ext="SCI"):
    ...
    rows = int(rows/2) * 2              # if odd number of rows, strip off the last one
    ...
    #print(hdulist['SCI'].data[:int(rows / 2)] == )
...
def outfiles(fname, splitrow, save_O=None, save_E=None, display=False):
    ...
    with pyfits.open(fname) as hdul:
        ...
        #O_beam.append(hdul['SCI'].copy())
        #E_beam.append(hdul['SCI'].copy())
        ...
        if (save_O is not None) and (save_E is not None):
