...
def get_files(
    ...
    if filenames == None:
        ...
        nfiles = Path(data_dir).glob(
            f"{prefix}*{'.' if extention else ''}{extention}"
        )
        ...
        for file in sorted(filenames):
            ...
            if Path(data_dir, file).is_file():
                ...
                errMsg = (
                    f"{file} not found in the data directory "
                    "{data_dir}, file unlisted."
                )
                logging.warn(errMsg)
    ...
    if nfiles == []:
        if filenames == None:
            errMsg = (
                f"Wildcard search, '{data_dir}/{prefix}*"
                f"{'.' if extention else ''}{extention}', "
            )
        else:
